...
namespace Org.BouncyCastle.Crypto.Macs
{
    ...
    public class Poly1305
    {
        ...
        private void SetKey(byte[] key, byte[] nonce)
        {
            ...
            uint t0 = LE_To_UInt32(key, 0);
            uint t1 = LE_To_UInt32(key, 4);
            uint t2 = LE_To_UInt32(key, 8);
            uint t3 = LE_To_UInt32(key, 12);
        ...
        {
            unsafe {
                fixed (byte* currentBlock = this.currentBlock) {
                    ulong t0 = LE_To_UInt32(currentBlock, 0);
                    ulong t1 = LE_To_UInt32(currentBlock, 4);
                    ulong t2 = LE_To_UInt32(currentBlock, 8);
                    ulong t3 = LE_To_UInt32(currentBlock, 12);
        ...
        {
            if (output.Length < outOff + BlockSize)
                throw new Exception("Output buffer is too short.");
            ...
            unsafe {
                fixed (byte* pOutput = output) {
                    UInt32_To_LE((uint)f0, pOutput, outOff);
                    f1 += (f0 >> 32);
                    UInt32_To_LE((uint)f1, pOutput, outOff + 4);
                    f2 += (f1 >> 32);
                    UInt32_To_LE((uint)f2, pOutput, outOff + 8);
                    f3 += (f2 >> 32);
                    UInt32_To_LE((uint)f3, pOutput, outOff + 12);
                }
            }
            Reset();
            return BlockSize;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal unsafe static uint LE_To_UInt32(byte* bs, int off)
        {
            return (uint)bs[off]
                | (uint)bs[off + 1] << 8
                | (uint)bs[off + 2] << 16
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal unsafe static void UInt32_To_LE(uint n, byte* bs, int off)
        {
            bs[off] = (byte)(n);
            bs[off + 1] = (byte)(n >> 8);
            bs[off + 2] = (byte)(n >> 16);
